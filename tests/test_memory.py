...
def test_memory_rotation_absolute():
    ...
    pool = WarmPool(max_tasks=50, keep_spare=True, max_memory=1)
    try:
...
def test_memory_rotation_percent():
    ...
    pool = WarmPool(max_tasks=50, keep_spare=True, max_memory_percent=0.0)
    try:
        pid1 = pool.run(get_pid, 5.0)
        pid2 = pool.run(get_pid, 5.0)
        assert pid2 != pid1
...
def test_memory_no_rotation_under_limit():
    ...
    pool = WarmPool(max_tasks=50, keep_spare=True, max_memory=total)
    try:
        pid1 = pool.run(get_pid, 5.0)
...
def test_memory_rotation_without_spare():
    ...
    pool = WarmPool(max_tasks=50, keep_spare=False, max_memory=1)
    try:
        pool.run(allocate_memory, 5.0, megabytes=1)
...
def test_memory_both_limits_absolute_triggers():
    ...
    pool = WarmPool(max_tasks=50, keep_spare=True, max_memory=1, max_memory_percent=1.0)
    try:
        pid1 = pool.run(get_pid, 5.0)
        pool.run(allocate_memory, 5.0, megabytes=1)
        pid2 = pool.run(get_pid, 5.0)
...
def test_last_memory_rss_populated():
    ...
    pool = WarmPool(max_tasks=50, keep_spare=True, max_memory=1024 * 1024 * 500)
    try:
        pool.run(add, 5.0, a=1, b=2)
...
def test_last_memory_rss_none_when_disabled():
    ...
    pool = WarmPool(max_tasks=50, keep_spare=True, max_memory_percent=None)
    try:
        pool.run(add, 5.0, a=1, b=2)
...
def test_memory_rotation_zero_limit():
    ...
    pool = WarmPool(max_tasks=50, keep_spare=True, max_memory=0)
    try:
        pid1 = pool.run(get_pid, 5.0)
        pid2 = pool.run(get_pid, 5.0)
        assert pid2 != pid1
